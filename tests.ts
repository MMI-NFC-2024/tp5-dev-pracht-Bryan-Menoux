...
const pingousinsMales = penguins.filter(d => d.sex === "MALE");
console.log("• filter() - Nombre de mâles:", pingousinsMales.length);

const pingouinsLourds = penguins.filter(d => d.body_mass_g != null && d.body_mass_g > 5000);
 ...
 */
const nomsEspeces = penguins.map(d => d.species);
...
const pingouinsParSexe = Object.groupBy(penguins, d => d.sex || 'inconnu');
